...
\begin{figure}[htbp]
  ...
  \caption[Distribution of ICESat-2 active subglacial lakes from 2018-2020]{
    ...
    % Blue lines are modelled subglacial water flux from \citet{LeBrocqEvidenceiceshelves2013}.
    % Purple dots are active subglacial lakes detected by Cryosat-2 radar altimetry from \citet{SiegfriedThirteenyearssubglacial2018}.
    % Orange dots are active subglacial lakes detected by ICESat-2 laser altimetry (this work).
...
Modelling conducted by \citet{CarterAntarcticsubglaciallakes2017} suggests that the water of active subglacial lakes is more likely stored in soft sediment which would not show as a bright specular surface in \gls{RES} data. % TODO Cite also (Schroeder et al., 2013)?
...
Over space and time, these subglacial drainage structures can change between the two extremes of efficient and inefficient regimes (Fig.~\ref{fig:4.2}) which has important consequences for ice dynamics (Sect.~\ref{sec:influenceofwateroniceflow}) % \citep{MullerVelocityfluctuationswater1973}
...
% The chapter concludes with an spatiotemporal analysis of a cascading pattern of subglacial lake drainage and filling at our Whillans Ice Stream study area from Nov 2019 to Sep 2020, and a discussion about the prevalence of multi-lobe lakes and what they can tell us about subglacial hydrology and the substrate.
...
% TODO add
% http://booksite.elsevier.com/9780123694614/PPT/08~Chapter_08.ppt
% Illustration of the main processes adding or removing mass from a vertical column through the ice thickness: surface and basal specific mass balances and divergence of flux (the case here shows positive divergence and hence removal of mass). Schematic adapted from Whillans (1977).
...
% where the total change in ice surface elevation $dh$ is equal to the change in elevation due to surface processes $dh_s$ plus the change in elevation due to basal processes $dh_b$.
...
% TODO insert figure of some slowly accumulating/mass wasting region in Antarctica with time on x-axis and elevation on y-axis

% From https://science.sciencemag.org/content/368/6496/1239
% \begin{figure}[htbp]
%   \includegraphics[width=1.0\textwidth]{chp4_fig_icesat_mass_loss.jpg}
%   \caption{
%     Mass loss from Antarctica (2003 to 2019) \citep{SmithPervasiveicesheet2020}.
%   }
%   \label{fig:4x}
% \end{figure}
...
The pattern of sudden elevation change over a short timescale caused by subglacial water volume changes typically occurs over a limited geographical region of a few square kilometres \citep[e.g.][]{SiegfriedEpisodicicevelocity2016}. %  ?TODO fact check + more citations over other regions of Antarctica
...
However, we need to account for temporal dependence when interpreting rate of elevation change $\frac{dh}{dt}$ at any given point.
...
Instead of $\sim131$ active subglacial lakes with a combined area of $\sim\SI{25793}{\kilo\metre\squared}$ detected over 13 years \citep{SiegfriedThirteenyearssubglacial2018}, we have detected $\sim159$ active subglacial lakes with a combined area of $\sim\SI{12372}{\kilo\metre\squared}$ over 2 years (Fig.~\ref{fig:4.1}). % TODO update numbers
Elevation anomaly clusters are especially prevalent along the Siple Coast (Fig.~\ref{fig:4.3}) which will be explored next.
% with 4 on Mercer Ice Stream, 15 on Whillans Ice Stream, 7 on Kamb Ice Stream and 4 on MacAyeal Ice Stream.
...
Further South along the Siple Coast, more active subglacial lake/elevation anomaly clusters are identifiable, with some forming mega-clusters.
...
% The results of the DBSCAN algorithm on the dhdt input over Whillans Ice Stream is presented visually in Fig~\ref{fig:4.4}.

% \begin{figure}[htbp]
%   \begin{subfigure}[t]{0.9\textwidth}
%     \includegraphics[width=1.0\textwidth]{chp4_fig1a_plot_atl11_dhdt_whillans}
%     \caption{Raw dhdt points}
%     \label{fig:4.4a}
%   \end{subfigure}
%   \vfill
%   \begin{subfigure}[t]{0.9\textwidth}
%     \includegraphics[width=1.0\textwidth]{chp4_fig1b_subglacial_lake_clusters_at_Whillans}
%     \caption{Subglacial lake clusters}
%     \label{fig:4.4b}
%   \end{subfigure}
%   \caption{
%     Finding subglacial lake point clusters using Density-based spatial clustering of applications with noise (DBSCAN).
%     (a) Input raw ATL11 points with elevation change over time (dhdt) values.
%     (b) Detected subglacial lake clusters, with draining lakes in red and filling lakes in blue, over the ICESat-2 2018-2020 time period.
%   }
%   \label{fig:4.4}
% \end{figure}
...
% Crossover anomaly
\begin{figure}[htbp]
  \includegraphics[width=1.0\textwidth]{chp4_fig_crossover_anomaly_whillans_7_2018-10-14_2020-09-30}
...
% 3D plot
\begin{figure}[htbp]
  \includegraphics[width=0.7\textwidth]{chp4_fig_dsm_whillans_7_cycles_3-8}
...
Subsequently from Feb 2020 to Sep 2020, the ice surface elevation decreased gradually over time.
...
% Crossover anomaly
\begin{figure}[htbp]
  \includegraphics[width=1.0\textwidth]{chp4_fig_crossover_anomaly_whillans_ix_2018-10-14_2020-09-30}
...
% 3D plot
\begin{figure}[htbp]
  \includegraphics[width=0.7\textwidth]{chp4_fig_dsm_whillans_ix_cycles_3-8}
...
% Crossover anomaly
\begin{figure}[htbp]
  \includegraphics[width=1.0\textwidth]{chp4_fig_crossover_anomaly_subglacial_lake_whillans_2018-10-14_2020-09-30}
...
% 3D plot
\begin{figure}[htbp]
  \includegraphics[width=0.7\textwidth]{chp4_fig_dsm_subglacial_lake_whillans_cycles_3-8}
...
% Crossover anomaly
\begin{figure}[htbp]
  \includegraphics[width=1.0\textwidth]{chp4_fig_crossover_anomaly_lake_12_2018-10-14_2020-09-30}
...
% 3D plot
\begin{figure}[htbp]
  \includegraphics[width=0.7\textwidth]{chp4_fig_dsm_lake_12_cycles_3-8}
...
% \begin{figure}[htbp]
%   \includegraphics[width=1.0\textwidth]{chp4_fig_crossover_anomaly_whillans_x_2018-10-14_2020-09-30}
%   \caption{}
...
\begin{figure}[htbp]
  \includegraphics[width=1.0\textwidth]{chp4_fig_cascade_whillans_ice_stream}
...
We interpret this as a cascading pattern of lake drainage (see Fig.~\ref{fig:cascade}) similar to that shown in Recovery Glacier \citep{DowDynamicsActiveSubglacial2018} and Thwaites Glacier \citep{SmithConnectedsubglaciallake2017}. % TODO check if there are the right citations for cascading drainage
...
% Our data ends at 30 Sep 2020 and no further trend can be described.
...
At Whillans 7 (Fig.~\ref{fig:whillans_7_dsm}), Subglacial Lake Whillans (Fig.~\ref{fig:subglacial_lake_whillans_dsm}) and to some extent in Whillans IX (Fig.~\ref{fig:whillans_ix_dsm}), we observed active subglacial lake clusters with multiple lobes closely spaced together, separated by ridge structures \SIrange{1}{4}{\kilo\metre} apart.
Similar patterns lobe-like patterns have been observed at the Southern catchment of Whillans Ice Stream at Subglacial Lake Conway and Subglacial Lake Mercer (Siegfried \& Carter, 2020, GRL in review). % TODO cite when paper comes out
...
We present another tool, using repeat measurements from ICESat-2 altimetry, to map out a potential locations of such enlongated subglacial features at Whillans Ice Stream. % TODO and Slessor Glacier?
...
This ICESat-2 subglacial lake time series extends that of ICESat and ongoing Cryosat-2 missions \citep{SiegfriedThirteenyearssubglacial2018}.
